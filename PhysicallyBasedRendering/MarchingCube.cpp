	...
	float reso)
{
	...
	cout << "scalar field num: " << scalarField.size() << endl;
}

void MarchingCube::ComputeParticleDensity(GLfloat* particlePoses, const int particleNum)
{
	particleDensities = new float[particleNum];

	OPENMP_FOR for (int i = 0; i < particleNum; i++)
	{
...
float* MarchingCube::ComputeScalarFieldUsingSphericalKernel(GLfloat* particlePoses, const int particleNum)
{
	...
	OPENMP_FOR for (int itr = 0; itr < particleNum; itr++)
	{
		...
		for (int node = 0; node < nodeNum; node++)
		{
			...
			if (glm::distance(particlePos, pos) <= this->h && particleDensities[i] >= 0.0001f)
			{
				float value = ComputePoly6(glm::distance(particlePos, pos)) / particleDensities[i];
...
float* MarchingCube::ComputeScalarFieldUsingAnisotropicKernel(GLfloat* particlePoses, const int particleNum)
{
	for (int i = 0; i < scalarField.size(); i++)
	{
		scalarField[i].value = 0.0f;
	...
	OPENMP_FOR for (int itr = 0; itr < particleNum; itr++)
	{
		...
		glm::mat3 R = glm::mat3(
			decomposedC.matrixU()(0, 0), decomposedC.matrixU()(1, 0), decomposedC.matrixU()(2, 0),
			decomposedC.matrixU()(0, 1), decomposedC.matrixU()(1, 1), decomposedC.matrixU()(2, 1),
			decomposedC.matrixU()(0, 2), decomposedC.matrixU()(1, 2), decomposedC.matrixU()(2, 2));
		...
		*/
		...
		for (int node = 0; node < nodeNum; node++)
		{
			...
			// anisotropic
			if (glm::distance(averagedPos, position) <= this->h && particleDensities[i] >= 0.0001f)
			{
				//cout << "dist: " << glm::distance(averagedPos, position) << endl;
				float value = AnisotropicSmoothingKernel(position - averagedPos, G) / particleDensities[i];
	...
	float* retScalarField = new float[(nodeNumX + 1)*(nodeNumY + 1)*(nodeNumZ + 1)];
	OPENMP_FOR for (int i = 0; i < (nodeNumX + 1)*(nodeNumY + 1)*(nodeNumZ + 1); i++)
	{
		retScalarField[i] = scalarField[i].value;
	}
	return retScalarField;
