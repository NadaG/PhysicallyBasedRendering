...
float* LoadDDS(const char * path, int *w, int *h, int *c)
{
	...
	unsigned width = hdr.dwWidth;
	unsigned height = hdr.dwHeight;
	unsigned texelsize = hdr.dwPitchOrLinearSize / width;

	*w = width;
	*h = height;
	// c는 채널 갯수
	*c = texelsize / 4;
	// texelSize값은 16인데 이것은 16바이트를 의미하는 걸까?

	float *ret = new float[width * height * texelsize / 4];
	// 4바이트는 32비트이고 
	// 한 텍셀 한 채널당 32비트가 할당되었기 때문??
	fread(ret, width * height * texelsize, 1, f);
